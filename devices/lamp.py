...
class ManageDevice(device_pb2_grpc.ManageDeviceServicer):
    def command(self, request, context):
        ...
        if request.device_name != DEVICE_NAME:
            ...
            if(order == "poweron"):
                if(current_status == order):
                    return device_pb2.CommandReply(device_name=DEVICE_NAME, response='Lamp already on.')
                current_status = order

            elif (order == "poweroff"):
                if(current_status == order):
                    return device_pb2.CommandReply(device_name=DEVICE_NAME, response='Lamp already off.')
                current_status = order
            else:
        ...
        return device_pb2.CommandReply(device_name=DEVICE_NAME, response=message)
